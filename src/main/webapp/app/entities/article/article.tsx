  ...
  return (
    <div>
      <h2 id="article-heading">
        <Translate contentKey="eCommerceJhipsterV1App.article.home.title">Articles</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="eCommerceJhipsterV1App.article.home.createLabel">Create new Article</Translate>
        </Link>
      </h2>
        ...
        {articleList && articleList.length > 0 ? (
            ...
            <tbody>
              {articleList.map((article, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{article.prix}</td>
                  <td>{article.typeArticleId ? <Link to={`type-article/${article.typeArticleId}`}>{article.typeArticleId}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="eCommerceJhipsterV1App.article.home.notFound">No Articles found</Translate>
            </div>
          )
        )}
      </div>
